...
public class Mensajes implements Serializable {
  ...
  public String validarJugada() {
    ...
    while (!valida) {
      if (jugada.equalsIgnoreCase("x")) {
        ...
        valida = true;
      } else if (((jugada.matches("[A-Fa-f1-6]*") && jugada.length() == 2)
          && (!jugada.equalsIgnoreCase("x")))) {
        jugada = jugada.toUpperCase();
        if (jugada.charAt(0) == '1'
            || jugada.charAt(0) == '2'
            || jugada.charAt(0) == '3'
            || jugada.charAt(0) == '4'
            || jugada.charAt(0) == '5'
  ...
  public void cubosRestantes(VentanaTablero vt, Juego juego) {
    JOptionPane.showMessageDialog(
        vt, "Cubos restantes : " + juego.getJugadorActual().getFichasRestantes(), " Cubos ", 3);
  }
  ...
  public static String letters() {
    ...
    while ((!letter.matches("[A-Za-z ]*")) || (letter.isEmpty())) {
      System.out.println(
          "Ingrese el dato ingresando unicamente letras , sin nigun caracter especial :");
      letter = in.nextLine();
      letter = letter.trim();
    }
    return letter;
  ...
  private void menu(Sistema sistema) {
    ...
    do {
      ...
      if (opcion == 4 && sistema.getJugadores().isEmpty()) {
        ...
        System.out.println(
            "No se puede jugar modo versus porque no hay ningun jugador registrado en el sistema.");
        opcion = 6;
