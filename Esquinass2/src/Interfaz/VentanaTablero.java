...
public class VentanaTablero extends javax.swing.JFrame implements Serializable {
  ...
  public void cuentaRegresiva() {
    ...
    if (this.ppt.perdiste()) {
      ...
      this.getJuego().getJugadorActual().setFichas(Collections.emptyList());
      this.setMusica(java.applet.Applet.newAudioClip(getClass().getResource("/musica/wasted.wav")));
      this.musica.play();
      JOptionPane.showMessageDialog(
          this,
          this.getJuego().getJugadorActual() + " perdiste por tiempo.",
          "Se acabo el tiempo",
          1);
      if (this.getJuego().getJugadorActual().equals(this.getJuego().getJugador1())) {
        ...
        this.getJuego()
            .getJugador1()
            .setPartidasPerdidas(this.getJuego().getJugador1().getPartidasPerdidas() + 1);
        this.musica.stop();
        ...
        this.getJuego()
            .getJugador2()
            .setPartidasPerdidas(this.getJuego().getJugador2().getPartidasPerdidas() + 1);
        this.musica.stop();
        this.getVm().setVisible(true);
        this.dispose();
      }
  ...
  public VentanaTablero(boolean time, Sistema sistema, Juego juego, String music, VentanaMenu vm) {
    ...
    this.setReanudada(
        this.getJuego().getJugador1().getFichasRestantes() < 25
            || this.getJuego().getJugador2().getFichasRestantes() < 25);
    this.pj = false;
  ...
  private void initComponents() {
    ...
    tiempo.setIcon(
        new javax.swing.ImageIcon(
            getClass().getResource("/Imagenes/reloj-de-arena.png"))); // NOI18N
    jPanel1.add(tiempo);
    tiempo.setBounds(860, 180, 120, 70);

    panelJuego.setBorder(
    ...
    guargarPartida.setIcon(
        new javax.swing.ImageIcon(
            getClass().getResource("/Imagenes/disco-flexible.png"))); // NOI18N
    guargarPartida.setText("Guardar Partida");
    guargarPartida.addActionListener(
        new java.awt.event.ActionListener() {
          public void actionPerformed(java.awt.event.ActionEvent evt) {
    ...
    silencio.setIcon(
        new javax.swing.ImageIcon(
            getClass().getResource("/Imagenes/reproductor-de-musica.png"))); // NOI18N
    silencio.setText("Silenciar / Reproducir Musica");
    silencio.addActionListener(
        new java.awt.event.ActionListener() {
          public void actionPerformed(java.awt.event.ActionEvent evt) {
    ...
    gano.setIcon(
        new javax.swing.ImageIcon(getClass().getResource("/Imagenes/winner.gif"))); // NOI18N
    jPanel1.add(gano);
    ...
    jLabel5.setIcon(
        new javax.swing.ImageIcon(getClass().getResource("/Imagenes/fondot.gif"))); // NOI18N
  ...
  private void guargarPartidaActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_guargarPartidaActionPerformed
    if (tiem) {
      this.timer.cancel();
    }
    ...
    JOptionPane.showMessageDialog(
        this,
        "Partida guardada con exito, para reanudarla ingrese a la opcion reanudar partida.",
        "Se guardo la partida",
        1);
    this.getVm().setVisible(true);
    this.dispose();
  } // GEN-LAST:event_guargarPartidaActionPerformed

  private void silencioActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_silencioActionPerformed
  ...
  private void JugarVsHumano(int fila, int columna) {
    ...
    if (getJuego().getJugador1().tieneFichas() && getJuego().getJugador2().tieneFichas()) {
      quienJuega.setText(getJuego().getJugadorActual().getAlias());
      cubos.setText(getJuego().getJugadorActual().getFichasRestantes() + "");
      if (this.getJuego().getJugadorActual().equals(this.getJuego().getJugador1())) {
      ...
      if (getJuego().getJugador1().getPuntaje() > getJuego().getJugador2().getPuntaje()) {
        ...
        getJuego()
            .getJugador2()
            .setPartidasPerdidas(getJuego().getJugador2().getPartidasPerdidas() + 1);
        ...
        JOptionPane.showMessageDialog(
            this,
            " Gana "
                + getJuego().getJugador1().getAlias()
                + " con "
                + getJuego().getJugador1().getPuntaje()
                + " puntos!\nPierde "
                + getJuego().getJugador2().getAlias()
                + " con "
                + getJuego().getJugador2().getPuntaje()
                + " puntos.",
            " Fin de partida ",
            1);
        this.vm.setVisible(true);
        ...
        getJuego()
            .getJugador1()
            .setPartidasPerdidas(getJuego().getJugador1().getPartidasPerdidas() + 1);
        this.gano.setVisible(true);
